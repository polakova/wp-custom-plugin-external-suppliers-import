...
/**
 * Michelin supplier importer
 * Based on ext-stock-michelin.php
 */
class Pneupex_Michelin_Importer extends Pneupex_Supplier_Importer_Base {

	public function __construct() {
		parent::__construct( 'Michelin' );
	}
	...
	protected function download_file(): ?string {
		// Credentials from constants
		$host     = defined( 'PNEUPEX_MICHELIN_FTP_HOST' ) ? PNEUPEX_MICHELIN_FTP_HOST : 'ftp.pexstore.sk';
		$username = defined( 'PNEUPEX_MICHELIN_FTP_USER' ) ? PNEUPEX_MICHELIN_FTP_USER : '';
		$password = defined( 'PNEUPEX_MICHELIN_FTP_PASS' ) ? PNEUPEX_MICHELIN_FTP_PASS : '';
		$base_dir = defined( 'PNEUPEX_MICHELIN_FTP_DIR' ) ? PNEUPEX_MICHELIN_FTP_DIR : 'michelin';
	...
	protected function parse_csv( string $file_path ): array {
		...
		if ( ( $handle = fopen( $file_path, 'r' ) ) !== false ) {
			while ( ( $row = fgetcsv( $handle, 1000, ';', '"', '\\' ) ) !== false ) {
				...
				// EAN in column 3, Quantity in column 6
				if ( ! empty( $row[3] ) ) {
					$products[] = $row;
				}
			}
	 ...
	 * Michelin uses product's own price from WooCommerce, multiplied by coefficient
	 */
